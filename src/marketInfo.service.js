(function () {
    ...
    angular
        ...
        .factory('marketInfoSrv', [
            '$log', '$q', 'call2ApiSrv', 'translateSrv',
            function ($log, $q, call2ApiSrv, translateSrv) {
                ...
                self.getInfo = function (marketId, _forceReload) {
                    ...
                    if (newMarkets.length > 0 || _forceReload || expiredMarkets.length > 0) {//market is not cached or need to refresh it
                        if (isSearching[marketIds[0]]) {
                            ...
                            call2ApiSrv.makePostSignCall('market/information', {}, {
                                ...
                                retVal = _.find(self.MarketInfo, function (market) {
                                    return (_.indexOf(marketIds, market.MarketId.toString()) > -1);
                                });
                ...
                /**
                 * @name updatePrice
                 * @description Updates market info price
                 * @returns {Array}
                 */
                self.updatePrice = function (market) {
                    var index = _.findIndex(self.MarketInfo, function (o) {
                        return o.MarketId == market.marketId;
                    });

                    self.MarketInfo[index].buyPrice = market.buy;
                    self.MarketInfo[index].sellPrice = market.sell;

                    return self.MarketInfo[index];
                };
                ...
                /**
                 * @name setLastPrice
                 * @description Save last pushed price
                 * @param  {object} marketInfo
                 */
                self.setLastPrice = function(price) {
                    if (!self.MarketInfo) { return; };
                    self.MarketInfo[0].LastPrice = price;
                }


                /**
                 * @name getLastPrice
                 * @description Get last pushed price
                 * @param  {object} marketInfo
                 * @return {price}
                 */
                self.getLastPrice = function() {
                    return self.MarketInfo[0].LastPrice || 0;
                };
